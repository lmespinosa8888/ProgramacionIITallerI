...
def pedir_doctor(cola_atencion):
    while True:
        print("\nDoctores disponibles:")
        mostrar_doctores()
        opcion = pedir_opcion("Seleccione un doctor: ", len(doctores))
        doctor = obtener_doctor(opcion)
        citas_pendientes = sum(
            cita.doctor is doctor
            for cita in cola_atencion
        )
        if doctor.tiene_cupo(citas_pendientes):
            return doctor
        print(
            f"La agenda de {doctor.nombre} está llena. "
            "Seleccione otro doctor."
        )
...
def registrar_cita(cola_atencion):
    print("\n========== REGISTRO DE CLIENTE ==========")
    cedula = pedir_numero(
    ...
    return Cita(usuario, pedir_doctor(cola_atencion))
...
# Cola FIFO para registrar citas en orden de llegada.
def ejecutar_programa():
    ...
    while continuar.upper() == "S":
        cita = registrar_cita(cola_atencion)
        cola_atencion.append(cita)
        print(f"\nCliente {cita.usuario.nombre} registrado correctamente.")
        continuar = input("\n¿Desea registrar otro cliente? (S/N): ")

    citas = procesar_citas(cola_atencion)
    clientes = deque(cita.usuario for cita in citas)
